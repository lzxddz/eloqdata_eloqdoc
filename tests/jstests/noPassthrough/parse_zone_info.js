...
(function() {
    ...
    let conn = MongoRunner.runMongod({timeZoneInfo: "tests/jstests/libs/config_files/bad_timezone_info"});
    ...
    conn = MongoRunner.runMongod({timeZoneInfo: "tests/jstests/libs/config_files/missing_directory"});
    assert.eq(conn, null, "expected launching mongod with bad timezone rules to fail");
    ...
    conn = MongoRunner.runMongod({timeZoneInfo: "tests/jstests/libs/config_files/good_timezone_info"});
