...
(function() {
    'use strict';
    load("tests/jstests/replsets/rslib.js");
    ...
    var s = new ShardingTest({
        ...
        mongos: 1,
        shards: 0,
        other: {keyFile: "tests/jstests/libs/key1", chunkSize: 1, enableAutoSplit: false},
    });
    ...
    d1.startSet({keyFile: "tests/jstests/libs/key2", shardsvr: ""});
    d1.initiate();

    print("d1 initiated");
    var shardName = authutil.asCluster(d1.nodes, "tests/jstests/libs/key2", function() {
    ...
    d1.stopSet();
    d1.startSet({keyFile: "tests/jstests/libs/key1", restart: true});
    d1.initiate();
    ...
    d2.startSet({keyFile: "tests/jstests/libs/key1", shardsvr: ""});
    d2.initiate();
    d2.awaitSecondaryNodes();

    shardName = authutil.asCluster(d2.nodes, "tests/jstests/libs/key1", function() {
    ...
    if (numDocs != num) {
        ...
        load('tests/jstests/libs/trace_missing_docs.js');
    ...
    authutil.asCluster(d1.nodes, "tests/jstests/libs/key1", function() {
        d1.awaitReplication();
    });
    authutil.asCluster(d2.nodes, "tests/jstests/libs/key1", function() {
        d2.awaitReplication();
    });
