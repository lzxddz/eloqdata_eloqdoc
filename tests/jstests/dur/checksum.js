...
resetDbpath(path);
mkdir(path + '/journal');
copyFile("tests/jstests/libs/dur_checksum_good.journal", path + "/journal/j._0");
...
resetDbpath(path);
mkdir(path + '/journal');
copyFile("tests/jstests/libs/dur_checksum_bad_last.journal", path + "/journal/j._0");
...
resetDbpath(path);
mkdir(path + '/journal');
copyFile("tests/jstests/libs/dur_checksum_bad_first.journal", path + "/journal/j._0");
conn = startMongodWithJournal();
var db = conn.getDB('test');
...
mkdir(path + '/journal');
copyFile("tests/jstests/libs/dur_checksum_bad_first.journal", path + "/journal/j._0");
copyFile("tests/jstests/libs/dur_checksum_good.journal", path + "/journal/j._1");
